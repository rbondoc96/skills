...
4. **Choose two reviewers.** Use orchestration to resolve reachable model + executor routes, then pick two from **different verified model providers**. The current host counts only when its provider is known and its native review route is callable. An executable on `PATH` proves only that an executor exists; it does not prove model entitlement or provider independence. OpenCode Go is a service, not a verified model-provider identity, until route provenance is recorded. If fewer than two distinct providers are reachable, stop and report the missing route.
