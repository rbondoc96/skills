...
- Use strict equality (`===`/`!==`) only; never `==`/`!=` (non-negotiable).

## Member and property ordering

Ordering convention (strong recommendation, not auto-corrected):

- **Object/type properties, constructor params, function options, component props** — sort by name in **ABC order**. When the shape mixes data and callbacks (e.g. event handlers), make two groups — data properties first, then callbacks — and sort ABC within each group.
- **Class members** — group by visibility in this order: `public`, `public static`, `protected`, `private`. Sort method names ABC within each group.
...
type CreateUserInput = {
  ...
  // callbacks, ABC
  onConflict: () => void;
  onCreated: (id: UserId) => void;
};
```

Finer logical-ordering preferences (prop-type grouping, member specifiers) may apply beyond the above. Follow the established ordering in the file being edited; when a file is inconsistent or new, apply the above and flag anything ambiguous.
...
- A native `enum` where a POJO `as const` belongs.
- `==`/`!=` instead of `===`/`!==`.
- Properties/props out of ABC order, or data and callbacks interleaved.
