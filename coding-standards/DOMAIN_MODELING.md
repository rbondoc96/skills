...
- Prefer string-literal unions and discriminated unions for closed sets of variants.
- **Never declare a native TypeScript `enum`** (non-negotiable). When a named-constant object is wanted, use a POJO `as const`:
