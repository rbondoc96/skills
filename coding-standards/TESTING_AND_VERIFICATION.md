...
Use representative runtimes for platform contracts that depend on runtime-specific APIs, serialization, or structured clone. Node-only tests are not proof of another runtime's behavior.
...
- **"Node tests cover all runtime code."** Only when the behavior does not depend on a different runtime's APIs or serialization.
