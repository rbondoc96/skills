...
When a user supplies a `postplan.dev` URL, fetch the uploaded HTML immediately with the shell. Do not use websearch or browser to retrieve it.
...
- In script-free files, give external linkes `target="_blank"` and `rel="noopener noreferrer"`. If any script exists, omit `target="_blank"`.
...
- Label them as `A`, `B`, `C`... for easy selection.
